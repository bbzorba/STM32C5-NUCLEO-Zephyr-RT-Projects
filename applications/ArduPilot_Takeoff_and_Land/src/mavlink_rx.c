...
static void mav_dispatch(uint32_t msgid, const uint8_t *payload)
{
	...
	case 0:
		...
		g_ap_hb.type      = payload[4];
		g_ap_hb.autopilot = payload[5];
		g_ap_hb.base_mode = payload[6];
		printk("[RX-HB] type=%s  autopilot=%s  armed=%s  base_mode=0x%02x\n",
		       mav_type_str(g_ap_hb.type),
		       mav_autopilot_str(g_ap_hb.autopilot),
		       (g_ap_hb.base_mode & 0x80) ? "YES" : "NO",
		       g_ap_hb.base_mode);
		break;
